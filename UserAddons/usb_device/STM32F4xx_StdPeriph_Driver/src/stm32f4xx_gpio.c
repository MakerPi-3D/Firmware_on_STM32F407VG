...
/**
  ...
  *                      x can be (A..I) to select the GPIO peripheral for STM32F42xxx/43xxx devices.
  *                      x can be (A, B, C, D and H) to select the GPIO peripheral for STM32F401xx devices.   
  * @param  GPIO_InitStruct: pointer to a GPIO_InitTypeDef_OLD structure that contains
  *         the configuration information for the specified GPIO peripheral.
  * @retval None
  */
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef_OLD* GPIO_InitStruct)
{
  uint32_t pinpos = 0x00, pos = 0x00 , currentpin = 0x00;

  /* Check the parameters */
  assert_param(IS_GPIO_ALL_PERIPH(GPIOx));
  assert_param(IS_GPIO_PIN_OLD(GPIO_InitStruct->GPIO_Pin));
  assert_param(IS_GPIO_MODE_OLD(GPIO_InitStruct->GPIO_Mode));
  ...
  for (pinpos = 0x00; pinpos < 0x10; pinpos++)
  {
    ...
    if (currentpin == pos)
    {
      ...
      if ((GPIO_InitStruct->GPIO_Mode == GPIO_Mode_OUT) || (GPIO_InitStruct->GPIO_Mode == GPIO_Mode_AF))
      {
        ...
        assert_param(IS_GPIO_SPEED_OLD(GPIO_InitStruct->GPIO_Speed));
...
/**
  ...
  * @param  GPIO_InitStruct : pointer to a GPIO_InitTypeDef_OLD structure which will be initialized.
  * @retval None
  */
void GPIO_StructInit(GPIO_InitTypeDef_OLD* GPIO_InitStruct)
{
...
void GPIO_PinLockConfig(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin)
{
  ...
  /* Check the parameters */
  assert_param(IS_GPIO_ALL_PERIPH(GPIOx));
  assert_param(IS_GPIO_PIN_OLD(GPIO_Pin));
...
void GPIO_PinAFConfig(GPIO_TypeDef* GPIOx, uint16_t GPIO_PinSource, uint8_t GPIO_AF)
{
  ...
  assert_param(IS_GPIO_AF_OLD(GPIO_AF));
